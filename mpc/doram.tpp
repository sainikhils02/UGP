...
{
    std::cout << "READ!!! (in the Additive Secret Sharing Scheme!!)" << std::endl; 
    if (shared_index.ctx == nullptr)
        throw std::runtime_error("Missing MPCContext in Locoram::Ref::read");
...
template<typename leaf_t, typename node_t, typename prgkey_t>
boost::asio::awaitable<leaf_t>
Locoram<leaf_t, node_t, prgkey_t>::Ref::update(const AShare<uint64_t>& shared_index,
                                               const AShare<uint64_t>& value_share) {
    if (shared_index.ctx == nullptr)
...
template<typename leaf_t, typename node_t, typename prgkey_t>
boost::asio::awaitable<void>
Locoram<leaf_t, node_t, prgkey_t>::update(const AShare<uint64_t>& shared_index,
                                          const AShare<uint64_t>& value_share) {
    if (shared_index.ctx == nullptr)
